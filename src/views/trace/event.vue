<template>
  <div class="page-container">
    ...
    <div class="search-bar">
      <el-input v-model="searchForm.keyword" placeholder="搜索零部件编码" clearable style="width:220px"
        @clear="fetchEvents" @keyup.enter="fetchEvents">
    ...
    <el-table :data="eventList" v-loading="loading" class="data-table">
      ...
      <el-table-column prop="eventName" label="事件名称" width="130" />
      ...
      <el-table-column label="区块链" width="120">
        <template #default="{ row }">
          <el-tooltip :content="row.txHash" placement="top">
            <el-tag type="success" size="small"><el-icon><SuccessFilled /></el-icon> 已上链</el-tag>
          </el-tooltip>
        </template>
      </el-table-column>
      ...
      <el-form ref="formRef" :model="form" :rules="rules" label-width="110px">
        <el-form-item label="零部件编码" prop="partsCode">
          <el-select v-model="form.partsCode" filterable style="width:100%" placeholder="选择零部件">
            <el-option v-for="p in allParts" :key="p.partsCode" :label="p.partsCode + ' - ' + p.partsName" :value="p.partsCode" />
          </el-select>
        </el-form-item>
    ...
    <el-drawer v-model="drawerVisible" title="事件详情" size="480px">
      <div v-if="currentEvent" class="event-detail">
        <div class="event-type-badge" :class="currentEvent.eventType">
          {{ eventTypeLabel(currentEvent.eventType) }}
        </div>
...
const form = reactive({
  partsCode: '', partsId: null, eventType: '', eventName: '',
  operatorName: '', location: '', eventTime: '', description: ''
})
...
const fetchEvents = () => {
  loading.value = true
  setTimeout(() => {
    const res = mockData.getMockData('/api/trace/event/list', 'get')
    let list = res.data.list
    if (searchForm.keyword) list = list.filter(e => e.partsCode.includes(searchForm.keyword))
    if (searchForm.eventType) list = list.filter(e => e.eventType === searchForm.eventType)
...
const openDialog = () => {
  ...
  Object.assign(form, { partsCode: '', partsId: null, eventType: '', eventName: '', operatorName: '', location: '', eventTime: '', description: '' })
...
const submitForm = async () => {
  ...
  await formRef.value.validate((valid) => {
    if (valid) {
      ...
      setTimeout(() => {
        const part = mockData.parts.find(p => p.partsCode === form.partsCode)
        const newEvent = {
          ...form, id: Date.now(), partsId: part?.id,
          operatorId: 1,
          blockHash: '0x' + Math.random().toString(16).slice(2, 14) + '...',
...
.chain-item {
  display: flex; gap: 12px; margin-bottom: 8px; font-size: 13px;
